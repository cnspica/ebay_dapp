...
const accountId = parseInt(new URLSearchParams(window.location.search).get('accountId'))
...
window.App = {
  start: function () {
    ...
    $('#bidding').submit(function (event) {
      ...
      EcommerceStore.deployed().then(function (i) {
        i.bid(parseInt(productId), sealedBid, {
          ...
          from: web3.eth.accounts[accountId],
          gas: 440000
        }).then(
          function (f) {
    ...
    $('#finalize-auction').submit(function (event) {
      $('#msg').hide()
      EcommerceStore.deployed().then(function (i) {
        i.finalizeAuction(parseInt(productId), {from: web3.eth.accounts[accountId], gas: 4400000}).then(
          function (f) {
            $('#msg').show()
...
function renderProductDetailsBak (productId) {
  EcommerceStore.deployed().then(function (i) {
  ...
  EcommerceStore.deployed().then(function (i) {
    i.getProduct.call(productId).then(function (p) {
      ...
      if (parseInt(p[8]) == 1) {
        ...
        $('#bidding').show()
      } else if (currentTime < (parseInt(p[6]) + 10)) { // 300秒的揭标时间
        $('#revealing').show()
      } else {
